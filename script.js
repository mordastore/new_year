// Делегирование клика по кнопке "открой свой подарок" (надёжно)
document.addEventListener('click', (e) => {
  ...
  if (openBtn) {
    ...
    if (cert) {
      cert.style.display = '';       // снимаем инлайн display:none
      cert.classList.add('show');    // плавная анимация из CSS
    }
    fireConfetti();
  }
});
...
// Конфетти в новогодней палитре (при клике)
function fireConfetti() {
  ...
  for (let i=0;i<count;i++){
    ...
    parts.push({
      ...
      r: 2 + Math.random()*4, col: colors[Math.floor(Math.random()*colors.length)],
